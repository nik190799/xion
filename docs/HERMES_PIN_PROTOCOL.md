...
For Genesis, Hermes is **doctrine-pinned, cast-verified, and adapter-lockfile-pinned**. `xion-verify hermes-runtime` verifies the Hermes commit, allowlist hash, disabled runtime flags, and the `requirements.lock` hash for the vendored `xion_hermes_runtime` adapter, while `xion-verify agent-souls` and `xion-verify agent-cast` verify that every live faculty was cast from a content-addressed Agent Soul against that pin. If upstream Hermes later exposes a stable installable package boundary, the adapter can be replaced through the same lockfile and verifier path.
