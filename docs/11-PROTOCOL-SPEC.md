...
| `GET`  | `/presence/state` | Current mood, palette, gesture mode (live as of Phase 6.4) |
| `GET`  | `/presence/stream` | Live scene-intent frames (SSE) (live as of Phase 6.4) |
| `GET`  | `/vitals` | Current vital-sign domains (live as of Phase 6.4) |
| `GET`  | `/me/receipts` | Verifiable inclusion proofs for signed interactions |
...
- `stream_visuals` — whether visual scene-intent frames stream to your client (warm default: on)
- `stream_vitals` — whether vital-sign changes push to your client (warm default: on)
- `stream_voice` — whether Xion's TTS engine generates audible voice for your session (warm default: off, requires explicit cost preview)
- `stream_text` — whether text is delivered via SSE stream or a single response block (warm default: on)
...
Returns governance-posted **per-message** price in XION (and optional USDC x402 equivalent), decomposed into the five slices from [`21-SUSTAINABILITY.md`](./21-SUSTAINABILITY.md): `variable_cost`, `overhead_slice`, `improvement_slice`, `reserve_slice`, `small_buffer`, each with last-reviewed UTC and vote id. Also includes a `modality_costs` map (`{ visuals: 0.0, vitals: 0.0, voice: <per-minute>, text_stream: 0.0 }`) so the user sees exactly what each stream-toggle costs.
