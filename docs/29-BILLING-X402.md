...
3. **Five-slice pricing transparency.** Governance posts a single per-message price decomposing into five slices ([`docs/07-ECONOMY.md`](./07-ECONOMY.md) § "Five-slice posted price"). Until Phase 5g-iii there was no live endpoint exposing those slices; `xion-verify pricing` was listed `NOT_YET_SEALED`. 5g-iii closes that gap with a `GET /pricing` endpoint and promotes the verifier from stub to live.
