...
**Phase 6.9 bounded-loop pin.** The primary turn loop distinguishes delegation depth from iteration count: `delegation_depth=1`, `iteration_count=3`, `tool_rounds=3`, `reasoning_tokens=4096`, and `wall_clock_s=8`. Tool schemas are supplied through the modular `ToolResolver` surface (`read_constitution`, `query_journal`, `query_sensorium`, `read_ledger`, `run_verifier`) and hosted Chutes turns use the canonical `generate_messages(..., tools=..., response_format=..., reasoning_effort="high")` surface. `xion-verify tool-resolver-mcp` and `xion-verify cognition-loop-bounded` verify this pin.
