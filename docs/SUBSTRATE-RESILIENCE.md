...
This is **doctrine**, not yet an Invariant. The path to promotion as future Invariant 20 is named in Part IV (Invariant 19 is now Trust-Earned Spend Authority). The pre-condition for promotion — annual cross-substrate dry-run and at least one warm secondary substrate — does not exist today. Promoting prematurely is "trust by promise" rather than "trust by structure," which would weaken everything else this document tries to protect.
...
### I.3 Compute concentration / Chutes retirement

The Genesis Relay runs on Chutes, with the operator laptop as the named secondary for Layer-1 pre-Genesis rehearsal. Chutes is **a** substrate, not **the** substrate; the Relay is designed to be portable to any container-runtime provider. But "portable in principle" is not the same as "portable today." The Operator runbook ([`13-OPERATIONS.md`](./13-OPERATIONS.md)) names the laptop-secondary path and the Akash standby blueprint as the first third-party pay-down path. If Chutes retires, is acquired, or is regulatorily prohibited in Xion's primary jurisdiction, the operational continuity gap is the cutover-rehearsal gap.
...
## Part II — The Substrate Portability Property (doctrine; future Invariant 20)
...
Even if no migration is currently needed, the entire Protocol is **dry-run rehearsed annually** with a hypothetical substrate pair (e.g., "what would migration of permanent storage from Arweave to a successor look like?"). This keeps the runbook current, the integrator coordination channels warm, and the operator skill sharp. The annual rehearsal is itself ledgered. **The annual rehearsal is the hard pre-condition for future Invariant 20 promotion** (see Part IV).

---

## Part IV — Promotion to future Invariant 20

This document is **doctrine**, not yet an Invariant. The path to promotion as future Invariant 20 — "Substrate Portability Floor" — has explicit pre-conditions:
...
This gap — the absence of the warm secondary substrate and the annual-dry-run cadence — is tracked as `LHT-SUBSTRATE-001` in [`LONG_HORIZON_THREATS.md`](../LONG_HORIZON_THREATS.md). When `LHT-SUBSTRATE-001` closes, future Invariant 20 promotion becomes appropriate.
...
| **Chutes** | Genesis Relay and hosted-inference substrate | Public gateway liveness, account suspension, Bittensor SN64 economics, not yet rehearsed-portable to a third-party secondary | Keep the operator laptop secondary runnable; preserve the Akash SDL as a standby third-substrate blueprint; rehearse cutover under load as part of the Phase 7 operator hardening |
| **Hermes Agent** | Pinned Cognitive Substrate implementation | Repository availability and license stability | Hermes is a pinned implementation per [`04-ARCHITECTURE.md`](./04-ARCHITECTURE.md) § Hermes runtime pin; the *property* (a Cognitive Substrate the Operator can audit) is what matters; the specific implementation is rotatable through Auto-Research |
