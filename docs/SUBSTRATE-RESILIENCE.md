...
The Genesis Relay runs on Chutes, with **Akash as the named secondary**. Chutes is **a** substrate, not **the** substrate; the Relay is designed to be portable to any container-runtime provider. But "portable in principle" is not the same as "portable today." The Operator runbook ([`13-OPERATIONS.md`](./13-OPERATIONS.md)) and [`AKASH_RELAY_DEPLOY.md`](./runbooks/AKASH_RELAY_DEPLOY.md) name the Akash path; the operator laptop is for local rehearsal only. If Chutes retires, is acquired, or is regulatorily prohibited in Xion's primary jurisdiction, the operational continuity gap is the cutover-rehearsal gap.
...
| **Chutes** | Genesis Relay and hosted-inference substrate | Public gateway liveness, account suspension, Bittensor SN64 economics, cutover to Akash must be rehearsed under load | Keep the **Akash secondary** warm; preserve additional tertiary options in the provider whitelist; rehearse cutover under load as part of the Phase 7 operator hardening |
