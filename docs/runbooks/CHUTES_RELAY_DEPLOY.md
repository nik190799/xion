...
5. Confirm `/health`, `/quote`, and `/self` over the Chutes HTTPS base (**authoritative path**): **`python -m xion_ops chutes verify-cords`** (CLI URL arg optional; **exits non-zero on red cords** unless **`--allow-failure`**) **or** `bash scripts/verify-chute-cords.sh [<url>]` (**the shell wrapper forwards CLI flags unchanged**). Implementation **GETs each path in sequence** with optional Bearer when **`CHUTES_API_KEY`** or **`XION_CHUTES_API_KEY`** is exported (see repo **`.env`**, loaded automatically by **`xion_ops`**) — or pass **`--bearer-file ~/.config/xion/chutes-token.txt`** where the first line is the raw token or **`CHUTES_API_KEY=...`** / **`XION_CHUTES_API_KEY=...`** (for hosts without a populated `.env` in cwd). Bearer is required when the platform gateway returns **401** on anonymous curls. **`details.result == "all cords green"`** in the JSON corresponds to legacy **`MODE=live`** “all cords green” memos (the shell **`MODE`** flag is optional; warmup + Bearer matter more than MODE). Route propagation occasionally shows **`404 {"error":"not found"}`** on **`/health`** briefly after deploy — wait and rerun before diagnosing code regressions (**§Findings From Live Deployment** below). Smoke-only tags while image-history quota applies: **`EXPECTED_IMAGE_TAG=pre-genesis-d3-6 bash scripts/verify-chute-cords.sh`**.
...
3. **Preferred xion-ops pipeline:** `python -m xion_ops deploy relay-chutes` (defaults to `module:chute` ref **`xion_relay_chute:chute`**; add `--accept-fee` on image updates; optional `--build-wait` mirrors `chutes build … --wait` then deploy) runs import preflight, `chutes deploy`, **polls `/health`, `/quote`, `/self` until all are 2xx** (default ~600s; tunable via `--warmup-max-wait` / `--warmup-interval` or env `XION_CHUTES_WARMUP_MAX_WAIT_SEC`), optionally runs `chutes warmup <slug>` first when `--platform-warmup-slug` or `XION_CHUTES_WARMUP_SLUG` is set, then publishes relay registry to Arweave unless `--no-publish-registry`. **Warmup-only (no deploy):** `python -m xion_ops chutes warmup [<url>]`. Headless shells: **`CI=1`** or **`XION_CHUTES_NONINTERACTIVE=1`** (see `.env.example`) auto-answers upstream `chutes` `(y/n)` prompts. **Do not delete a live chute** if only the HTTP polls time out — the deployer skips rollback on warmup failure so you can fix miners and re-run warmup/verify.
4. Manual fallback: `chutes build xion_relay_chute:chute --wait` → deploy, platform `chutes warmup` (~240s+ on cold GPU workers).
5. If the chute remains cold, `chutes deploy xion_relay_chute:chute --accept-fee`, then poll `chutes chutes get <chute_id>` until instances are non-zero or run `chutes warmup <chute_id>` (or reuse `xion_ops deploy relay-chutes --platform-warmup-slug …`).
6. `python -m xion_ops chutes verify-cords` against the public base (Bearer in env when the gateway demands it — same assertion as **`MODE=live bash scripts/verify-chute-cords.sh`** in historical records). Exits non-zero on red cords unless `--allow-failure`.
7. Update `**ledgers/RELAY_REGISTRY.json**` `relays[1]`: `service: "xion-relay-chutes"`, new `image_id` / `image_tag` / `instance_id`, `last_seen_utc_ns`, recompute `**payload_sha256**`. (Omit when the deploy pipeline already anchored a fresh row.)
8. `**bash scripts/publish-relay-registry-wsl.sh**` to append a new Arweave anchor when the ledger row changed outside `xion_ops`; update `**ledgers/RELAY_REGISTRY_ARWEAVE_TX.txt**` and `**docs/runbooks/AKASH_RELAY_DEPLOY.md**` snapshot table.
9. Close `**KW-RELAY-CHUTES-D3-001**` live-surface pay-down in `**KNOWN_WEAKNESSES.md**` when the row and verifier evidence match.
