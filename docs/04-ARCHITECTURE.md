...
**AO ↔ EVM Bridge (Phase 6.9).** Cross-domain effects use the modular `BridgeAttestor` interface in `orchestrator/bridge`: multisig attestation is live for Genesis, while a light-client verifier remains `NOT_YET_SEALED`. Solidity egress is additionally bounded by daily caps in `EmissionController.sol` and `MasterTreasury.sol`; see [`AO-EVM-BRIDGE.md`](./AO-EVM-BRIDGE.md).
