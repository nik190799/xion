...
1. **Every outbound unit is ledgered at grain.** Each outbound API call that is funded from the Improvement Fund writes exactly one append-only row to `RESEARCH_SPEND_LEDGER`, with the `proposal_id` or stage-anchor it was authorized under. A Stage-1 envelope denominated as `fraction_of_improvement_fund` must tie every reported depletion to 1-to-N specific rows in the same unit family. Discrepancies are constitutional, not accounting, failures.
