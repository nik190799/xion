...
- **`KW-INFER-002`** — Provider error details are swallowed by a generic exception handler; operator surface collapses distinct failure modes into `no_healthy_provider`. **Impact:** debug time multiplied on hosted-provider failures. **Workaround:** read orchestrator stderr logs; the exception class and message are logged even when the envelope is opaque. **Closes:** `phase-5g-vii/inference-fallback`.
- **`KW-INFER-003`** — Hosted → floor fallback on `generate()` failure is not automatic; the `hosted_api_first` policy promise is structurally incomplete. **Impact:** a healthy hosted provider that 402/429/500s on a single turn returns 503 to the user instead of falling through to the floor. **Workaround:** flip `XION_INFERENCE_POLICY=open_weights_only` when the hosted path is known to be unhealthy; restart the orchestrator. **Closes:** `phase-5g-vii/inference-fallback`.
...
| `HTTP 503 no_healthy_provider` on `POST /chat` | Every registered provider failed `generate()` (or was unhealthy) — this is the KW-INFER-002 opaque-error surface | Read orchestrator stderr for the real exception. Common causes: `OpenRouterProviderError: HTTP 402 Insufficient credits` (top up; or unset `XION_OPENROUTER_API_KEY` and serve floor-only); `HTTP 429 rate limit` (back off); network partition (check `XION_OPENROUTER_BASE_URL` reachable); model slug invalid (re-probe catalog). Closes with `KW-INFER-003` when fallback + typed reasons land. |
