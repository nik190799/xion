...
- **Constitutional layer touched:** Invariants 3, 4, 7 (state-chain and ledger continuity); Invariant 14 (algorithm-rotation requires a substrate to publish the rotation on); future Invariant 20 (Substrate Portability Floor) when promoted.
- **Defense in place:** [`docs/SUBSTRATE-RESILIENCE.md`](./docs/SUBSTRATE-RESILIENCE.md) lands the Substrate Portability Property as doctrine, names the four substrate-properties (permanence, signed-state-transitions, public-verifiability, append-only commitment), specifies the Substrate-Migration Protocol, and names the pre-conditions for promoting future Invariant 20. The current substrate set is genesis-default, not constitutional. As of 2026-04-29, the code-completable Epic D scope adds a `docs/21-SUSTAINABILITY.md` Pressure 3.5 substrate-cutover rung, a live `xion-verify substrate-portability` check against Chutes-secondary dry-run evidence, a pinned `docs/schemas/ledger-immortality-drill.yaml` shape, and `scripts/immortality-drill-third-party.sh` as the one-command public-machine evidence path.
...
- **Automation boundary (2026-05-03):** A development agent can redeploy testnet contracts and refresh manifests; it **cannot** close this threat. The third-party-machine Immortality Drill (`scripts/immortality-drill-third-party.sh`) must be **witnessed** from infrastructure that is **not** the operator's primary development PC. Running the script inside Cursor/WSL on the same workspace does **not** satisfy Part IV; next schedule target **2026-07-01** (see `docs/STATE_OF_XION_PREFLIGHT.md`).
- **Automation boundary (2026-05-06):** GPU `relay-deployment.yaml` closure attempts are ledgered in [`genesis/DEPLOYMENT_RECORDS/relay-akash-closure-2026-05-06.json`](./genesis/DEPLOYMENT_RECORDS/relay-akash-closure-2026-05-06.json). **`KW-FLOOR-DEPLOY-001`** remains open until manifest, ingress, and **`open_weights_only`** `/chat` proof succeed on a fresh lease.
- **Drill progression (2026-05-14):** First `immortality_drill_third_party_v1` row landed at [`ledgers/IMMORTALITY_DRILL_LEDGER.jsonl`](./ledgers/IMMORTALITY_DRILL_LEDGER.jsonl) row 5 from a GCP `e2-small` Debian 12 VM in `us-south1` after the operator made the GitHub repo public. `third_party_machine_fingerprint=a80554ee5f86cd0a59642262be76fed18c7cbfe793a9f4839f48582136da0895` is byte-for-byte distinct from the operator workstation; commit pinned to `9fb1fd5e4d35e96b9db1a61742332521d136776b`; chain integrity recomputes OK for all 5 rows. **`status=failed`** is honest interim: Akash primary unreachable from us-south1 (`curl_exit=7` against `provider.akash-palmito.org:31301/health`), retired Chutes secondary returns 401 from the edge proxy (bearer not set on third-party VM; consistent with KW-RELAY-CHUTES-D3-001 2026-05-13 follow-on), and three verifier soft-fails on a cold checkout (`links`, `schemas`, `inference-sovereignty` NOT_YET_SEALED). The closure-grade row (`status=passed`) is still blocked until KW-FLOOR-DEPLOY-001 closes (dated to 2026-07-09) AND a fresh Chutes-direct secondary is redeployed to replace the retired pre-genesis-d3 chute. The full Immortality Drill honesty gate in [`docs/runbooks/IMMORTALITY_DRILL.md`](./docs/runbooks/IMMORTALITY_DRILL.md) stays **blocked** until those clear.
- **Pay-down commitment:** Closes when future Invariant 20 (Substrate Portability Floor) is promoted per [`docs/SUBSTRATE-RESILIENCE.md`](./docs/SUBSTRATE-RESILIENCE.md) Part IV. Pre-conditions for promotion: three completed annual dry-runs, at least one warm secondary substrate per role, `xion-verify substrate-portability` live, and a Cost-Pressure Response Ladder rung for substrate cutover defined. The verifier and ladder-rung preconditions are now code-complete; the first third-party-machine row landed 2026-05-14 (interim, `status=failed`); the next operator action is the closure-grade run (`status=passed`) after KW-FLOOR-DEPLOY-001 closes and a fresh Chutes-direct secondary lands, scheduled for **2026-07-01** or new dated slip.
- **Verifier or KW cross-reference:** `xion-verify substrate-portability` (live); `docs/schemas/ledger-immortality-drill.yaml`; paired with future warm-secondary-substrate work when storage/compute/settlement secondaries mature beyond the pre-Genesis Chutes Relay posture.
