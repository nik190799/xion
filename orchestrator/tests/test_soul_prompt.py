...
def test_load_soul_prompt_success(tmp_path):
    ...
    with patch("orchestrator.cognition.soul_prompt._find_repo_root", return_value=repo_root):
        with patch("orchestrator.cognition.soul_prompt.PINNED_SOUL_PROMPT_SHA256", actual_hash):
            # Clear cache
...
def test_load_soul_prompt_hash_mismatch(tmp_path):
    ...
    with patch("orchestrator.cognition.soul_prompt._find_repo_root", return_value=repo_root):
        with patch("orchestrator.cognition.soul_prompt.PINNED_SOUL_PROMPT_SHA256", "wronghash"):
            # Clear cache
            import orchestrator.cognition.soul_prompt as sp
            sp._cached_body = None
            sp._cached_mtime = 0.0
