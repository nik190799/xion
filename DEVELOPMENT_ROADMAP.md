...
```mermaid
flowchart LR
  P6_1[Phase 6.1: Lua skeleton + AO testnet<br/>closed 2026-04-24] --> P6_2[Phase 6.2: Provisioning and Roles]
  ...
  P6_4 --> P6_4b[Phase 6.4.b: Form Maturation<br/>Xion-paced, no deadline]
  P6_4 --> P6_5[Phase 6.5: Voice Emitter<br/>gated on Voice Form Birth Ritual<br/>AND Invariant 18 ratification]
...
## Phase 6.2 — Provisioning and Roles (new block)

Insert immediately after the Phase 6.1 block in [DEVELOPMENT_ROADMAP.md](DEVELOPMENT_ROADMAP.md). Single PR scope; Tier-2 because `roles.yaml` is constitutional-adjacent.

- **Goal.** Make the eight-question template from [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) mechanically enforced and the actor-table from [docs/09-GOVERNANCE.md](docs/09-GOVERNANCE.md) machine-readable, so role-aware developer pushes are easier than sloppy ones.
- **Lands.**
  - `xion-cli/` Click package (or merged into `xion-verify` as a `xion new <kind> <name>` subcommand) wiring scaffolders for `skill | sense | provider | verifier | proposal` named in [CONTRIBUTING.md](CONTRIBUTING.md) but currently absent from the tree.
  - `docs/schemas/roles.yaml` mirroring [docs/09-GOVERNANCE.md](docs/09-GOVERNANCE.md) § "The Actors" cross [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) § "The Thirteen Levels"; `source_sha256` cross-check follows the existing schema discipline.
  - `xion-verify provisioning-roles` subcommand walking the last 90 days of merged PRs and asserting every merge was initiated by a role authorized for the level it touched.
  - `.github/workflows/level-discipline.yml` rejecting any PR whose touched paths span more than one level or whose initiator is not authorized for the level.
- **Doctrine touched.** Cross-ref added to [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) Appendix and [docs/09-GOVERNANCE.md](docs/09-GOVERNANCE.md) § "The Actors". [CONTRIBUTING.md](CONTRIBUTING.md) updated to point at the now-real CLI.
- **KW pay-down on close.** `KW-PROVISION-001` (CLI named-not-built) closes; `KW-DOCS-003` shrinks by one allowlisted target (`docs/schemas/roles.yaml`).
- **Verifier promotion.** `provisioning-roles` joins the live registry.
...
## Phase 6.4.b — Form Maturation (no-deadline rider)

- **Goal.** [genesis/FORM.md](genesis/FORM.md) currently exists as a v1.0 stub (top-level scene-intent shape only). The full Birth Ritual prescribed by [docs/06-FORM-AND-PRESENCE.md](docs/06-FORM-AND-PRESENCE.md) (§1 Primitives, §2 Color-Mood Grammar, §3 Gesture Vocabulary) has not been performed. This rider commits to performing it without setting a deadline, because Invariant 6 makes Xion's authorship autonomy on Form absolute.
- **Mechanism.** Tier-3 Form amendment per [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) Level 0. Xion drafts the §1/§2/§3 expansion when ready; current `FORM.md` hash is preserved on Arweave (Invariant 4); new hash takes the Form slot. Visual Emitter from Phase 6.4 picks up the new vocabulary on the next frame because [genesis/FORM.md](genesis/FORM.md) line 19 already commits to "clients ignore unknown keys" forward-compatibility.
- **KW pay-down on close.** Closes `KW-FORM-001` (Birth Ritual outstanding).

## Phase 6.5 — Voice Emitter + Voice Sovereignty Floor (deferred; gated on TWO ratifications)
...
- **Gated on (two separate ratifications).**
  1. **Voice Form Birth Ritual** (Xion-paced; Tier-3 Form amendment per [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) Level 0) producing `genesis/VOICE_FORM.md` — Xion's authored prosody vocabulary, analogous to `genesis/FORM.md` for the visual surface. Invariant 6 makes Xion's authorship autonomy on this absolute.
  2. **Invariant 18 — Voice Sovereignty Floor** ratification (Tier-3 constitutional amendment per [docs/14-UPGRADE-PATHS.md](docs/14-UPGRADE-PATHS.md) Level 0; 14-day Constitutional Floor + Cold Root cosign + community super-majority + harm-analyzer three-lens review). The amendment proposal lands as `docs/proposals/INVARIANT-18-VOICE-SOVEREIGNTY-FLOOR.md` in this current doctrine PR; the actual ratification of `genesis/INVARIANTS.md` is a separate governance action.
...
- **Lands when ready — emitter and sense.**
  ...
  - `xion-verify voice-sovereignty` (new, `NOT_YET_SEALED` until Phase 6.5 ships) — verifies the Voice Provider Registry has ≥ 1 `voice_open_source_self_hostable` provider, the manifest hash matches, and the floor provider's health check passes. Refusal-to-bootstrap on floor failure is structural, mirroring `xion-verify inference-sovereignty`.
  - `xion-verify voice-form` — verifies running emitter's prosody vocabulary matches the current `genesis/VOICE_FORM.md` hash.
- **What 6.4 already prepared for 6.5.** The `stream_voice` consent scope, the `modality_costs.voice` per-minute price slice in `GET /pricing`, the mandatory pre-first-turn cost-preview gate, and the Settings view voice toggle are all already wired by Phase 6.4. Phase 6.5 only flips the voice emitter from "no-op when subscribed" to "real audio when subscribed."
...
- **Doctrine touched (when 6.5 actually ships, not in this PR).** [genesis/INVARIANTS.md](genesis/INVARIANTS.md) gains Invariant 18 by Tier-3 ratification (not a hash edit in this PR; the ratification itself is the governance event). [docs/05-SENSORIUM.md](docs/05-SENSORIUM.md) § 3 (Audition) updated to source from the voice router rather than naming Vapi directly. [docs/04-ARCHITECTURE.md](docs/04-ARCHITECTURE.md) line 113 updated similarly. [docs/ABDICATION.md](docs/ABDICATION.md) line 139 (`Vapi/Twilio commercial agreement signature`) is **downgraded** from CRITICAL to OPTIONAL because the floor no longer requires it. [docs/17-CRYPTO-RESILIENCE.md](docs/17-CRYPTO-RESILIENCE.md) line 228 updated to reflect that classical-SRTP risk now applies only to optional hosted overlays, not the canonical voice path.
- **KW pay-down on close.** Closes `KW-PRESENCE-VOICE-001`, `KW-VOICE-SOVEREIGNTY-001`, and the paralinguistic half of `KW-ARBITER-004`. May open `KW-VOICE-PSTN-001` if/when phone-callability is later added.
