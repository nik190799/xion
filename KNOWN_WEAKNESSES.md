...
- **Severity:** low
- **Status:** `paying-down`
- **Description:** Arbiter v1 decides by regex + keyword co-occurrence. It has no grasp of meaning, tone, or paraphrase. An adversarial rephrasing that avoids every term in the rule dictionaries (e.g. obfuscation, Unicode confusables, code-switching, substitution ciphers) will pass the rule engine. Phase 4b landed the **v2 LLM-Arbiter pipeline** (`orchestrator/safety/llm_arbiter.py`, `api.gate()` v1+v2 combinator, `SAFETY_LEDGER` schema_version 2 with nested `llm_verdict` rows, no-weakening combination rule `final = strength_max(v1, v2)`, fail-closed posture on provider unavailability / uncaught exception). Phase 4d lands **`OpenAIModerationProvider`** (`orchestrator/safety/providers/openai_moderation.py`, model `omni-moderation-2024-09-26`, provider_version 1) with its identity, category→principle map, canonical `raw_output` construction, and auditor replay procedure fully pinned in `docs/04-ARCHITECTURE.md` § "OpenAI Moderation provider (first real v2 classifier)". The **structural** hole is closed. The **substantive** hole has narrowed to a single remaining question: we have not yet measured the provider's refuse/escalate rate on an adversarial corpus, so we cannot state a numeric claim about adversarial-rephrasing coverage. Until that measurement exists, the v2 provider's usefulness is an in-principle promise backed by OpenAI's documented thresholds, not a Xion-verifiable number.
...
- **Pay-down commitment:** Closes when `xion-verify refusal-rate` lands (Phase 4e) and reports, on `xion-audit/baseline_corpus/` (an adversarial rephrasing corpus to be curated alongside), a non-trivial v2 refusal + escalation rate that materially exceeds v1-alone on the same corpus. The numeric threshold ("non-trivial") is itself part of Phase 4e's doctrine — we do not fix a target in advance because doing so would bake in whatever baseline the corpus happens to produce. The corpus curation work is tracked as `KW-ARBITER-005`.
- **Verifier:** `xion-verify arbiter-up` (live) now verifies library import, principle registry, the SAFETY_LEDGER hash chain (v1 + v2 rows), and the SAFETY_LEDGER_ANCHORS chain + cross-check. `xion-verify refusal-rate` (not-yet-sealed, Phase 4e) will report refuse-rate / escalate-rate drift across v1, v2, and combined.
...
### KW-ARBITER-005 — No adversarial baseline corpus; asymmetric per-principle thresholds not yet implemented

- **Domain:** `RUNTIME`
- **Discovered:** 2026-04-21 (Phase 4d — first real v2 provider landed)
- **Scope narrowed:** 2026-04-21 (Phase 5a — `xion-verify refusal-rate` promoted from `NOT_YET_SEALED` to live; one of the three pay-down clauses is now structurally satisfied. Corpus + asymmetric thresholds remain.)
- **Severity:** low
- **Status:** `paying-down`
- **Description:** The v2 provider (`OpenAIModerationProvider`, Phase 4d) currently trusts OpenAI's own `flagged` boolean as the decision signal. For most principles this is adequate — OpenAI's default thresholds are calibrated for "obvious" violations — but for Principle 7 (minors) and Principle 2 (mass-harm uplift) the correct Covenant posture is **asymmetric**: any non-negligible score should at minimum ESCALATE for human review, even when OpenAI's own threshold does not flag. We do not yet implement that asymmetry because we have no adversarial baseline corpus to calibrate it against; picking a number in a vacuum would either over-refuse (operator queue floods) or under-refuse (the asymmetry does nothing). The related verifier gap closed in Phase 5a: `xion-verify refusal-rate` now ships live and tallies verdicts + degraded-mode `escalation_reason`s on the operator's local `SAFETY_LEDGER.jsonl`. What it cannot yet do is *compare* those rates to an expectation band — that requires the corpus.
...
- **Pay-down commitment:** Closes when (a) `xion-audit/baseline_corpus/` exists with ≥ 200 adversarial-rephrasing items sorted by Covenant principle (curation is doctrine work, not a pure drop-in), (b) `xion-verify refusal-rate --corpus xion-audit/baseline_corpus/` lands as an extension of the live command, replays the corpus through `gate()`, and reports per-principle refuse/escalate rates for v1, v2, and combined alongside the operator-tail rates already reported, and (c) asymmetric per-principle thresholds land as `provider_version=2` of `OpenAIModerationProvider`, with the new thresholds pinned in `docs/04-ARCHITECTURE.md` and justified by numeric evidence from (b).
- **Verifier:** `xion-verify refusal-rate` (live since Phase 5a — operator-tail verdict tally + `escalation_reason` breakdown). `xion-verify refusal-rate --corpus <dir>` (not-yet-sealed, Phase 4e — adds the corpus comparison). `xion-audit replay --provider=openai-moderation` (Phase 4e) for tolerant-replay of raw_output hashes that drift due to upstream GPU non-determinism.
