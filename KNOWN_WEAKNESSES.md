...
- **Discovered facts (recorded so the next maintainer does not relearn them, each tied to a specific build):**
  ...
  6. (`pre-genesis-d3-6` warmed smoke deployment) The currently deployed smoke image is warm and verifies green with `EXPECTED_IMAGE_TAG=pre-genesis-d3-6 bash scripts/verify-chute-cords.sh`: `/health`, `/quote`, and `/self` return 200 OK with the expected envelope; `/pricing` returns the Chutes platform GPU-pricing payload before it reaches the chute.
- **Mitigations:**
  ...
  - The cord pipeline itself is now proven end-to-end on the live Chutes platform: build → push → schedule → miner assignment (verified instance) → public 200 OK on all three smoke cords.
...
- **Pay-down commitment:** Closes in two steps.
  1. **Cord pipeline closure:** green on the currently deployed `pre-genesis-d3-6` smoke image via `EXPECTED_IMAGE_TAG=pre-genesis-d3-6 bash scripts/verify-chute-cords.sh`. The newer root deploy module `xion_relay_chute.py` remains the actual Chutes CLI target (`xion_relay_chute:chute`) and is pinned to `pre-genesis-d3-7`, but `chutes build xion_relay_chute:chute --wait` is blocked until the Chutes 24-hour image-history quota clears. Once the rebuild succeeds, deploy, warm, re-run `scripts/verify-chute-cords.sh` without the d3-6 override, then update `ledgers/RELAY_REGISTRY.json` with the chute id / public URL / image id row and publish to Arweave.
