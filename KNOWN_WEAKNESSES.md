...
- **Mitigations:**
  ...
  - **2026-05-03:** Base Sepolia `MasterTreasury` was **redeployed from current source** and **pinned** in `genesis/TREASURY_VAULTS.json` (`master_treasury` / deploy tx / block updated). JSON-RPC soak works via `treasury_soak_probes.py` when `cast` is not on Windows PATH. **Does not substitute an external audit** — status remains `open` until pay-down (a) or explicit Sprint residue (b).
