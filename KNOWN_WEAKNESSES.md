...
- **Mitigations:** The method fails closed and names this weakness. Sepolia D3 rotation rehearsals run through `BaseEvmService.cast_send()` because the testnet governance authority is an EOA. Mainnet Safe actions remain manual through the Safe UI until a real client is added. EOA owner funding is tracked by `genesis/FUNDING_TARGETS.json` and `xion-verify funding-balances`.
- **Pay-down commitment:** Close before any mainnet warm-tier rotation or treasury operation depends on automated Safe proposal creation. Closure requires a real Safe Transaction Service client behind `BaseEvmService.safe_propose_tx()`, offline tests for payload construction, and live dry-run evidence against Base Sepolia.
