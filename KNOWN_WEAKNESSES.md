...
- **Status:** `paying-down`
- **Description:** The pre-genesis Chutes Relay deployment at `https://nikhilkadalge-xion-relay-pre-genesis-d3.chutes.ai` (chute id `89866bfc-5ddd-5382-b887-116d8901808f`) currently runs a *smoke build*. The latest deployed image is `pre-genesis-d3-6` (image id `971ea1ac-1850-5b26-86af-bc0aa23c7f06`). After WSL setup was fixed, `EXPECTED_IMAGE_TAG=pre-genesis-d3-6 bash scripts/verify-chute-cords.sh` passed against the warmed deployment: `GET /health`, `GET /quote`, and `GET /self` all returned deterministic envelopes tagged `service="xion-relay-chutes-smoke"`. `GET /pricing` is confirmed intercepted by the Chutes platform pricing proxy and is intentionally not a smoke cord. This remains an honest B4 smoke closure, not the full Relay surface.
...
- **Discovered facts (recorded so the next maintainer does not relearn them, each tied to a specific build):**
  ...
  8. (`2026-04-27` operator WSL) Akash mainnet lease `dseq=26563373` returned `ready_replicas=1` with forwarded HTTPS `https://provider.161.97.85.20.nip.io:30564` (see `lease-status --auth-type mtls`). `ledgers/RELAY_REGISTRY.json` `relays[0].endpoint` was set to that base; the Chute warmed again: `EXPECTED_IMAGE_TAG=pre-genesis-d3-6 bash scripts/verify-chute-cords.sh` all green, and `xion-verify substrate-portability` OK after a Chutes-secondary dry-run row (seq=2) was appended. Arweave publication remains blocked only by absence of a configured registry wallet JWK in the automation environment, not by endpoint liveness.
- **Mitigations:**
  ...
  - The cord pipeline itself has been proven end-to-end on the live Chutes platform: build → push → schedule → miner assignment (verified instance) → public 200 OK on all three smoke cords. Current cold-state must be re-warmed and re-verified before registry publication.
...
- **Pay-down commitment (updated 2026-04-26 for Akash-primary registry order):** `xion-verify discovery` now requires **Akash** at `relays[0]` and **Chutes** at `relays[1]`. The old “Chutes-only primary row first, then Arweave” story is replaced by: **(A)** a **real Akash lease HTTPS base** in `relays[0].endpoint` (not `…-pending.invalid`), **(B)** `python3 scripts/publish-relay-registry-arweave.py` with `XION_REGISTRY_WALLET_JWK_PATH` (writes `ledgers/RELAY_REGISTRY_ARWEAVE_TX.txt` and an Arweave tx id), and **(C)** re-run `xion-verify discovery` (still `NOT_YET_SEALED` on honest placeholder `public_key` material until keys are bound). The **Chutes** secondary row remains the d3-6 **smoke** disclosure until a new image is warm.
  1. **Registry + Arweave closure (genesis primary on Akash):** set the Akash row from a live `lease-status` forward (`bash scripts/closeout-genesis-akash-primary-wsl.sh` with `XION_AKASH_HTTPS_BASE` and Chutes secrets for the dry-run path), then publish the registry; record the Arweave tx in `CHANGELOG.md` and `docs/runbooks/AKASH_RELAY_DEPLOY.md` § *Arweave registry snapshot*.
