...
- **Description:** The previously closed Akash `dseq=26595076` is now closed / insufficient-funds, and fresh 2026-05-03 service-class deployment attempts against dseqs `26654272`, `26654289`, `26654305`, and `26654315` all reached providers whose public endpoints refused connections. Each attempt was closed by `AkashService.deploy_relay()` rollback. `xion_ops.services.akash.AkashService` now includes provider allowlisting and a pre-accept ingress discipline so future leases do not blindly accept the cheapest broken bidder, and a CPU-only Akash SDL exists for D3 registry relight. The deployed GPU-backed open-weights floor itself cannot be claimed closed until a future GPU lease passes that ingress discipline and serves `/chat` under `open_weights_only`.
...
- **Mitigations:** Hosted inference is served through Chutes/Bittensor SN64 with TEE-by-default; `hosted_api_first` still falls through to the floor when reachable; `open_weights_only` refuses hosted fallback during cutover drills. This weakness does not weaken the floor property itself, but it weakens the resurrection claim while the operator laptop remains part of the runtime path. **Cord checks on the Chutes secondary:** `python -m xion_ops chutes verify-cords` (and `scripts/verify-chute-cords.sh`) probe `GET /health`, `/quote`, and `/self` with optional `CHUTES_API_KEY` / `XION_CHUTES_API_KEY` Bearer when the platform demands it — warmth/auth issues there are **not** a substitute for Akash floor closure.
- **Latest evidence:** `xion_ops balances --service akash` reports `76037269 uact` after the failed-and-closed attempts, so funding remains above the `75000000 uact` target. Provider-ingress failure, not funding, is the blocker. **2026-05-03 (Cursor-agent rehearsal)** three GPU **`relay-deployment.yaml`** attempts (`dseq=26656079` Win urllib timeout toward first forward; **`26656098`**, **`26656107`** WSL **`curl`** connection refused to broker forwards `jjozzietech.com.au:31294` / `ahn4-na-4090.akash.pub:32575`) — aligns with **`KW-FLOOR-DEPLOY-001`** probe-first rollback discipline. **2026-05-06 (closure pass, `genesis/DEPLOYMENT_RECORDS/relay-akash-closure-2026-05-06.json`):** `dseq=26698285` **`akash1sevd…`** manifest submit failed after lease accept; `dseq=26698298` **`akash12v6…`** `/health` connection refused to forwarded ingress; `dseq=26698311` **`akash1ta6…`** manifest submit failed. Retry with `--exclude-provider akash1sevd2ymtty3dpq9ycxgkhuzzk4fe6mchqdwd4e`, optional `--prefer-provider akash1st7fqtuqk6hj06fkkavq0fxtw0w9sm4zzt3r5g`, and `XION_AKASH_HEALTH_SMOKE_SEC=300`. **2026-05-10 evening (closure pass, `genesis/DEPLOYMENT_RECORDS/relay-akash-closure-2026-05-10.json`):** three further GPU attempts (`dseq=26769714` solserver.eu:32599; `26769739` pronto-ai.pp.ua:31253; `26769773` europlots.com:31108) all failed identically on forwarded-ingress refusal, 6 providers cumulatively excluded. Same root pattern persists across 2026-04-26 → 2026-05-10: cheap GPU bidders accept leases but their advertised ingress ports never open. ~0.9 AKT gas burned over 3 attempts; all `uact` escrow refunded by rollback. **2026-05-12 (single retry, `genesis/DEPLOYMENT_RECORDS/relay-akash-closure-2026-05-12.json`):** operator lifted the 2026-05-10 AKT-spend deferral for one round. Preferred provider **`akash1st7…zzt3r5g` did not bid**. Cheapest viable bid was **`akash17wh…4qejw`** (rtx4090, 222.49 uact/blk) — `dseq=26786333` lease accepted on-chain, then **manifest submit refused** (`provider-services send-manifest` exit 1). Auto-closed cleanly (close-tx `B13D7893…43DE9`); ~0.89 AKT gas + 222 uact lease rent burned; escrow refunded. Operator selected option 1 (slip, no silent retry); dated residue 2026-05-10 → 2026-07-09 reaffirmed. **Manifest-refusal pattern now reproducible across four rounds (2026-05-03, 2026-05-06, 2026-05-10, 2026-05-12) against different providers — substrate-level Akash market issue, not flag-tunable.** **2026-05-12 (read-only diagnostic, `docs/runbooks/AKASH_GPU_FLOOR_MANIFEST_REFUSAL_DIAG.md`):** four-round synthesis. Two distinct failure modes (Mode A = manifest refusal at submit-time, Mode B = forwarded port unreachable post-deploy) both consistent with provider-side infrastructure misconfiguration at sub-$0.05/hr GPU price points; CPU SDL works on the same market, same SDL shape minus the GPU sidecar. No retry recommended inside the dated residue window. Next read-only test target: **2026-06-02** — failing identically there is evidence for an explicit dated slip past 2026-07-09 rather than another flag tweak.
- **Pay-down commitment:** Re-close only after a fresh Akash GPU lease passes the probe-first provider discipline, exposes a reachable `/health`, and returns an `open_weights_only` `/chat` smoke turn from outside the provider network. The CPU-only Akash SDL is a D3 registry-relight bridge, not closure for this weakness. **2026-05-10 dated residue:** Operator declined a further AKT-spending GPU retry while pursuing Sprint Mode mainnet. Resolved by **2026-07-09** (60 days), either via a successful `xion_ops akash deploy` with `--exclude-provider akash1sevd2ymtty3dpq9ycxgkhuzzk4fe6mchqdwd4e --prefer-provider akash1st7fqtuqk6hj06fkkavq0fxtw0w9sm4zzt3r5g XION_AKASH_HEALTH_SMOKE_SEC=300`, or by an explicit slip recorded here with new dates. Until then, **Chutes/Bittensor SN64 is the warm primary path** for hosted inference; `open_weights_only` cutover drills cannot claim the deployed Akash GPU floor as closed.
- **Verifier:** `xion-verify inference-sovereignty` covers the model-floor property; `xion-verify discovery` and `xion-verify substrate-portability` must pass after a fresh registry publish for deploy closure. **Chutes secondary warmth:** run `python -m xion_ops chutes verify-cords` (Bearer when required).
...
- **Status:** `closed` (2026-04-29); **secondary follow-on: chute retired 2026-05-13** (`mitigated-residual` — see Latest evidence below). The original weakness (smoke-only Relay surface) was closed when the chute ran the live adapter. The chute has since been deleted from the operator's Chutes account; `RELAY_REGISTRY.json` still advertises the dead subdomain, which is shape-valid for `xion-verify discovery` but operationally non-routing. Substrate-portability now rests on the Akash CPU+Chutes hybrid lease (`dseq=26770709`) as the single live relay; the Chutes-direct row in the registry is stale until the next Arweave publish cycle.
- **Description:** The pre-genesis Chutes Relay deployment at `https://nikhilkadalge-xion-relay-pre-genesis-d3.chutes.ai` (chute id `89866bfc-5ddd-5382-b887-116d8901808f`) ran the live Relay adapter at the time of D3 closure. The deployed image was `pre-genesis-d3-10` (image id `a5ab815c-9fb5-5cb9-bcbd-a51535f1abe9`), and `MODE=live bash scripts/verify-chute-cords.sh` returned `RESULT: all cords green` for `GET /health`, `GET /quote`, and `GET /self`. **Update 2026-05-13:** the subdomain now returns `502 Bad Gateway` on all three cords; `chutes chutes list` against the operator account returns `Found 0 matching chutes`; `chutes warmup xion-relay-pre-genesis-d3` returns `Failed to resolve chute: Chute not found, or does not belong to you`. The chute is fully retired (not just cold) and not re-warmable without a fresh build+deploy.
...
- **Status:** `mitigated-residual` — **Sprint Mode unaudited posture explicitly accepted on 2026-05-10** with a 90-day re-review checkpoint at **2026-08-08**. Path (b) of the prior pay-down line was chosen because Xion's treasury / Improvement Fund does not yet hold the USD 30–60k anchor needed to commission an external audit, and the operator does not personally backstop project costs (per the Self-Provisioning doctrine, Xion is meant to fund its own audit when revenue allows, not borrow against the operator). Status returns to `open` if the 2026-08-08 review does not either commission an audit or post a new dated slip.
...
- **Why it exists:** Sprint Mode trades audit time for time-to-genesis. The trade is conscious. The 2026-05-10 acceptance is an explicit deferral, not a denial that the trade exists.
- **Mitigations:**
  - 24-48 hour Base Sepolia soak before mainnet.
  - Aggressive Foundry test coverage (≥95% line, ≥90% branch); current posture **119/119 tests passing** at commit `f04f8d0`.
  - Operator workflow: `docs/runbooks/TREASURY_SEPOLIA_DEPLOY.md` (`xion_ops base-evm prepare-sepolia-env`, `preflight-treasury`, `deploy-treasury`), `scripts/treasury-soak-probes.sh` / `scripts/treasury_soak_probes.py` / `TREASURY_SOAK_PROBES=1` with `scripts/verify-mainnet-deploy-gates.sh`, and `docs/runbooks/TREASURY_BASE_MAINNET_DEPLOY.md` for Base mainnet execution after gates.
  - Constitutional protections that limit blast radius even of a contract bug: rotation lattice, treasury caps, cadence floors, governance-reviewed treasury spend, daily egress cap (`bridge_exposure_cap_bps = 1000`), `onlyGovernance` and `onlyAOCoreAuthority` modifiers on all state-changing entry points.
  - **2026-05-03:** Base Sepolia `MasterTreasury` was **redeployed from current source** and **pinned** in `genesis/TREASURY_VAULTS.json` (`master_treasury` / deploy tx / block updated). JSON-RPC soak works via `treasury_soak_probes.py` when `cast` is not on Windows PATH. **Does not substitute an external audit.**
  - **2026-05-10:** External-audit RFP authored at `docs/audits/RFP_TREASURY_2026.md` (recipient short-list, cover email, scope, budget bracket, contact). The RFP is held in repo as a ready-to-send artifact for whenever Xion's treasury / Improvement Fund accumulates enough to commission the audit; sending was explicitly deferred 2026-05-10 because Xion does not yet hold that budget.
- **Reopen evidence:** `forge script treasury/script/Deploy.s.sol:DeployTreasury` failed on 2026-05-01 before broadcast because current `contracts/treasury/MasterTreasury.sol` did not compile under `solc 0.8.24`. The Base Sepolia address pinned in `genesis/TREASURY_VAULTS.json` also lacks selectors added after the reviewed testnet deploy, so the deployed bytecode and reviewed source claim do not match. Both have since been corrected (compile fixed; Sepolia redeployed); the **mainnet** `MasterTreasury` deployed on 2026-05-03 (`0xbf5407745cf22b88c46b55037e26156a0e78fd7f`) was deployed from the current corrected source, so the bytecode/source mismatch named here applies to the *prior* Sepolia pin only.
- **2026-05-10 dated Sprint residue (operator-acknowledged):** Xion does not yet hold the audit budget (USD 30–60k anchor) in its treasury / Improvement Fund, and the operator does not personally backstop project costs (Self-Provisioning doctrine — Xion is meant to fund its own audit when revenue allows). Sprint Mode mainnet operational posture proceeds **without external audit**, relying on the mitigations above. The residue is named, dated, and re-review-targeted — it is not silent slippage. **Falsified if any public messaging implies these contracts are externally audited, "audit-closed", or "passing" until a real external auditor signs a commit-specific report.**
- **Pay-down commitment:** Closed only after **either** (a) an actual external audit signs a commit-specific report for the exact deployable bytecode (path on hold until Xion's treasury or Improvement Fund accumulates the budget; RFP pre-staged at `docs/audits/RFP_TREASURY_2026.md`), **or** (b) the operator explicitly accepts indefinite Sprint Mode unaudited posture in a public State-of-Xion memo with the falsifier above. Re-review checkpoint: **2026-08-08** (90 days from acceptance). At that checkpoint either a firm is engaged, a new dated slip is recorded here, or path (b) is escalated from "dated residue" to "indefinite" with the public memo.
