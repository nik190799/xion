...
- **Discovered facts (recorded so the next maintainer does not relearn them, each tied to a specific build):**
  ...
  8. (`2026-04-27`–`2026-04-28` operator WSL) Same Akash lease and Chutes cord evidence as fact 7; **`ledgers/RELAY_REGISTRY.json`** was published to Arweave (**tx `vEvdNUQt…`**, then republished after Ed25519 pubkey binding as **`n6OCNc5mfsgDBdBOUYJsS7tYo980lNQnWgzJzDYdyqE`** — see **`ledgers/RELAY_REGISTRY_ARWEAVE_TX.txt`** for the current anchor). **`xion-verify discovery`** reads **`OK`** with non-placeholder `ed25519:` keys on both relay rows (operator one-shot: **`python scripts/gen-relay-registry-ed25519-pubkeys.py`**; private material only in **`secrets/relay_registry_ed25519.json`**).
- **Mitigations:**
